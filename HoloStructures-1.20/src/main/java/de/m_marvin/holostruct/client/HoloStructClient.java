...
public class HoloStructClient {
	...
	public static void onConnectoToServer(ClientPlayerNetworkEvent.LoggingIn event) {
		// TODO access level
		HoloStruct.CLIENT.LEVELBOUND.setAccess(new ClientLevelAccessorImpl(Minecraft.getInstance()), AccessLevel.FULL_CLIENT);
	}
	
	@SubscribeEvent
	public static void onDisconnectServer(ClientPlayerNetworkEvent.LoggingOut event) {
		HoloStruct.CLIENT.LEVELBOUND.setAccess(new NoAccessAccessor(), AccessLevel.NO_ACCESS);
	}
