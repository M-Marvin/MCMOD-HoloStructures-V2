...
public class PixelArtGenerator {
	...
	protected int calculateRGBfromMapColor(MapColor color, MapColor.Brightness brightness) {
		int bgr = color.calculateARGBColor(brightness);
	...
	public void resetLists() {
		...
		this.whitelistConfig.addAll(ClientConfig.DEFAULT_BLOCK_WHITELIST.get().stream().map(name -> BuiltInRegistries.BLOCK.get(ResourceLocation.parse(name))).map(o -> o.isEmpty() ? null : o.get().value()).toList());
		this.blacklistConfig.clear();
		this.blacklistConfig.addAll(ClientConfig.DEFAULT_BLOCK_BLACKLIST.get().stream().map(name -> BuiltInRegistries.BLOCK.get(ResourceLocation.parse(name))).map(o -> o.isEmpty() ? null : o.get().value()).toList());
	}
